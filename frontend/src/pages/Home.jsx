...
//     // Admin-configurable: how many services/blogs/topics show on the homepage
//     api
//       .get("/settings")
//       .then((r) => {
//         setHomeServicesCount(r.data.data?.homeServicesCount || 6);
//         setHomeBlogsCount(r.data.data?.homeBlogsCount || 3);
//         const topicsLimit = r.data.data?.homeTopicsCount || 4;
...
export default function Home() {
  ...
  useEffect(() => {
    // Safe API calls with default empty array fallbacks
    api.get("/banners")
    ...
    api.get("/technologies")
      .then((r) => setTechnologies(r.data?.data || []))
    ...
    api.get("/expertise")
      .then((r) => setExpertise(r.data?.data || []))
      .catch(() => setExpertise([]));
  ...
  // Safe index access using optional chaining
  const banner = banners?.[0];
    ...
    <>
      ...
      <section className="section">
        <div className="container">
          ...
          <h2>Quality IT Solutions & Digital Services</h2>
          <div className="grid-3">
            {(Array.isArray(expertise) && expertise.length
              ? expertise
              : [
                  {
                    _id: 1,
                    title: "Consulting",
                    description:
                      "Digital transformation and consulting services for scaling businesses.",
                  },
                  {
                    _id: 2,
                    title: "IT Services",
                  ...
                  },
                  {
                    ...
                    description:
                      "Establish a strong foothold in the online medium and grow your reach.",
                  },
                ]
            ).map((e) => (
              <div className="card" key={e._id}>
        ...
        <div className="container">
          ...
          <div className="grid-4">
            {(Array.isArray(services) && services.length
              ? services.slice(0, homeServicesCount)
              : []
            ).map((s) => (
              <div className="card service-card" key={s._id}>
                {s.imageUrl && <img src={s.imageUrl} alt={s.title} />}
        ...
        <div className="container">
          ...
          <div className="grid-3">
            {(Array.isArray(topics) && topics.length
              ? topics.slice(0, homeTopicsCount)
              : []
            ).map((t) => (
              <Link to={`/topics/${t.slug}`} className="card" key={t._id}>
                {t.featuredImage && (
            ...
            {(!topics || !topics.length) && (
              <p>
                Topics will appear here once added and enabled for the homepage
      ...
      <section className="section">
        <div className="container">
          ...
          <div className="grid-3">
            ...
            {(!blogs || !blogs.length) && (
              <p>Blog posts will appear here once added from the admin dashboard.</p>
            )}
          </div>
        </div>
      </section>
