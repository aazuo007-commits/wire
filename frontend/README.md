...
Latest updates:
- **Fixed a second crash**: `SyntaxError: "undefined" is not valid JSON`. This happened when a
  login attempt got back an unexpected response shape (e.g. from a misconfigured `VITE_API_URL`)
  and the app stored the literal string `"undefined"` in `localStorage`; on the next page load,
  `JSON.parse("undefined")` threw and crashed the whole app before it could even render. Login now
  refuses to store anything unless the response has both a token and an admin object, and the
  admin-session reader self-heals by clearing any already-corrupted value instead of crashing.
...
### Local vs. production `VITE_API_URL`

`frontend/.env` (used for local dev, already set up in this project) and `frontend/.env.example`
(the template) both keep the two values side by side so you can see which is which:

```bash
# Local development (active)
VITE_API_URL=http://localhost:5000/api

# Production — Render backend (swap this in, and comment out the line above, before deploying)
#VITE_API_URL=https://wire-backend-67ir.onrender.com/api
```

- **Locally**: keep the `http://localhost:5000/api` line active and run `npm run dev` as normal.
- **On Render**: `frontend/.env` is git-ignored and never gets deployed, so it has no effect on
  Render's build — instead, set `VITE_API_URL` as an actual **Environment Variable** in the
  Render Static Site's dashboard (Settings → Environment), using your backend's real URL, e.g.
  `https://wire-backend-67ir.onrender.com/api`. Environment variables set in Render's dashboard
  take priority and are what Vite actually bakes into the build.
- Whichever value you use, remember Vite reads `VITE_API_URL` at **build time** — changing it
  and just restarting the service does nothing; you need a fresh build/deploy.
