...
export default function Navbar() {
  ...
  // A single "which dropdown is open" id covers Services, Topics, and every dynamic parent menu.
  ...
  const topicsItems = topics.map((t) => ({ key: t._id, label: t.title, to: `/topics/${t.slug}` }));
      ...
      <div className="container navbar-inner">
        ...
        <nav className={`nav-links ${open ? "open" : ""}`}>
          ...
          {topicsItems.length > 0 && (
            <div
              className="nav-dropdown"
              onMouseEnter={() => setOpenMenuId("topics")}
              onMouseLeave={() => setOpenMenuId((id) => (id === "topics" ? null : id))}
            >
              <span className="nav-link-parent">
                <NavLink
                  to="/topics"
                ...
                <button
                  ...
                  onClick={() => setOpenMenuId((id) => (id === "topics" ? null : "topics"))}
                >
                  ▾
                </button>
              </span>
              <NavMegaMenu items={topicsItems} open={openMenuId === "topics"} onNavigate={closeAll} />
            </div>
          )}
